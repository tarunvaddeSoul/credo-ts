...
pub fn create_or_update_revocation_state(
    ...
    create_or_update_revocation_state_with_reader(
        &tails_reader,
        rev_reg_def,
        rev_status_list,
        rev_reg_idx,
...
// [credo-ts browser vendor patch] same as create_or_update_revocation_state, but generic over the
// tails accessor so wasm builds can use an in-memory reader instead of std::fs
pub fn create_or_update_revocation_state_with_reader<TA>(
    tails_reader: &TA,
    rev_reg_def: &RevocationRegistryDefinition,
    rev_status_list: &RevocationStatusList,
    rev_reg_idx: u32,
...
where
    TA: crate::cl::RevocationTailsAccessor,
{
